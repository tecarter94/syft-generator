    ...
    public UpdateControl<TaskRun> reconcile(TaskRun taskRun, Context<TaskRun> context) {
        ...
        if (isFailed(taskRun)) {
            ...
            if (isOomKilled(taskRun)) {
                ...
                orchestrator.handleUpdate(generationId, GenerationStatus.FAILED, "OOMKilled", null);
            } else {
                log.warn("TaskRun '{}' FAILED. Reason: TaskRun Failed", taskName);
                orchestrator.handleUpdate(generationId, GenerationStatus.FAILED, "TaskRun Failed", null);
            }
